...
public class Rekeningnummer {
    ...
    public Rekeningnummer(String nummer) {
        ...
        if (controleGetal < 2 || controleGetal > 98) { throw new IllegalArgumentException("1° of 2° cijfer verkeerd");
        } var teControlerenNummer = Long.valueOf(matcher.group(2) + "1114" + matcher.group(1));
        if (teControlerenNummer % 97 != 1) { throw new IllegalArgumentException("Nummer bevat verkeerde cijfers");
        }
        this.nummer = nummer;
    }
    @Override
